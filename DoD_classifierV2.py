...
# Import ollama for model communication
import ollama
...
def _setup_logging(self):
    ...
    # V V V THIS IS THE CORRECTED FUNCTION SIGNATURE V V V
# We change the default value from "gemma-3" to "mistral"

def send_to_model(self, prompt: str, model_name: str = "magistral:24B") -> str:
    """
    Sends a prompt to a specified Ollama language model.
    
    Args:
        prompt: Input prompt for the model
        model_name: Name of the Ollama model to use (defaults to "mistral")
        
    Returns:
        Generated text response as a string.
    """
    try:
        self.logger.info(f"Sending prompt to Ollama model: {model_name}")
        # The actual call to the Ollama API
        ...
        self.logger.info(f"Received {len(generated_text)} characters from Ollama.")
        return generated_text
    except Exception as e:
        self.logger.error(f"Error communicating with Ollama model {model_name}: {e}")
        # Return an empty string on failure so the quality check can discard it
        return ""
    ...
    def generate_domain_examples(self, 
        ...
        while len(examples) < num_examples and attempts < num_examples * max_retries:
            ...
            # Generate raw output
            raw_output = self.send_to_model(enhanced_prompt, model_name="GPT-OSS-120B")
...
# V V V PASTE THIS ENTIRE FUNCTION INTO THE MetacognitiveTrainingGenerator CLASS V V V
# A good place is right after the generate_domain_examples method.
